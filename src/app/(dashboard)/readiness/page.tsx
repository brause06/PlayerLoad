...
export default async function ReadinessBoardPage() {
  // Fetch all players (status field is currently used to store ACWR risk like 'OPTIMAL', 'HIGH', etc.)
  const players = await prisma.player.findMany({
    orderBy: { name: "asc" }
  });

  // Find the most recent session date in the DB to use as our "today"
  ...
  // Calculate readiness for all players
  const readinessData = await Promise.all(
    players.map(async (player: any) => {
      const acwrData = await calculateACWR(player.id, targetDate);
      const acwr = acwrData.acwr;

      // Fetch today's actual Wellness record (or the latest one close to today)
      // Since it's a demo, we'll try to get their most recent record.
      const wellnessRecord = await prisma.wellness.findFirst({
      ...
      if (acwr >= 0.8 && acwr <= 1.3) {
        finalScore += 25; // Optimal
      } else if (acwr > 1.5) {
        finalScore -= 15; // High Risk Load
      } else if (acwr < 0.8) {
        finalScore -= 5; // Under-loaded
      } else {
        finalScore += 10; // Acceptable but not optimal
      }
        ...
        <div className="mt-8">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {readinessData.map((player: any) => {
              // Get UI colors based on category
              ...
              return (
                <Link href={`/players/${player.id}`} key={player.id} className="block bg-[#131313] hover:bg-[#181818] border border-neutral-800/80 hover:border-neutral-700 transition-all rounded-xl p-5 group flex flex-col justify-between relative overflow-hidden cursor-pointer">
                  ...
                  {/* Card bottom border gradient hint */}
